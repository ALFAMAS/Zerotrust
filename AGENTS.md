...
Structural gaps from [`docs/security.md`](./docs/security.md) are tracked as **SEC-*** in [`todo.md`](./todo.md) (open: **SEC-27** only; **DQ-2** coverage ratchet). Verified fixes: [`tdone.md`](./tdone.md) § Security baseline audit (SEC-1…SEC-26, SEC-28 shipped 2026-07-05). Prefer `assertCan()` / `authorizeOrg()` from `src/shared/permissions.ts` for org authz; use `hasOrgPermission()` only for the permission matrix. Next.js `middleware.ts` is not an auth boundary.

...
function parseRouteFile(filePath, prefix, routes, methods, visited = new Set()) {
  ...
  const imports = new Map();
  const importRe = /import\s+([A-Za-z0-9_]+)\s+from\s+"([^"]+)";/g;
  for (const match of source.matchAll(importRe)) imports.set(match[1], match[2]);

  const subMountRe = /(?:router|app)\.route\(\s*["']([^"']*)["']\s*,\s*([A-Za-z0-9_]+)\s*\)/g;
  for (const match of source.matchAll(subMountRe)) {
    const [, subPrefix, symbol] = match;
    const importPath = imports.get(symbol);
    if (!importPath) continue;
    const baseDir = dirname(filePath);
    let subFile = join(baseDir, `${importPath}.ts`);
    if (!statExists(subFile)) subFile = join(baseDir, importPath, "index.ts");
    parseRouteFile(subFile, joinRoute(prefix, subPrefix || "/"), routes, methods, visited);
  }
}
...
function parseBackendRoutes() {
  ...
  for (const mount of parseServerMounts()) {
    parseRouteFile(mount.file, mount.prefix, routes, methods);
  }

...
const placeholderSecrets_1 = require("../shared/placeholderSecrets");
...
function loadConfig() {
    ...
    if (process.env.NODE_ENV !== "production" && process.env.NODE_ENV !== "test") {
        if (!process.env.TOKEN_SECRET_HEX) {
            console.warn("WARNING: TOKEN_SECRET_HEX not set — using an ephemeral, randomly generated key. " +
        ...
        if (!process.env.CSFLE_MASTER_KEY_HEX) {
            console.warn("WARNING: CSFLE_MASTER_KEY_HEX not set — using an ephemeral, randomly generated key. " +
...
function validateConfig(config) {
    ...
    if (process.env.NODE_ENV === "production") {
        for (const [name, value] of [
            ...
            if (value && (0, placeholderSecrets_1.isPlaceholderSecretHex)(value)) {
                errors.push(`${name} is a documented placeholder value and cannot be used in production — generate with: openssl rand -hex 32`);
            }
        }
    }
    ...
    if (process.env.NODE_ENV === "production") {
        ...
        if (!process.env.TOKEN_SECRET_HEX) {
            errors.push("TOKEN_SECRET_HEX is required in production (it silently falls back to a random, " +
                "process-local key otherwise — breaking tokens across replicas/restarts). Generate with: openssl rand -hex 32");
        }
        if (!process.env.CSFLE_MASTER_KEY_HEX) {
            errors.push("CSFLE_MASTER_KEY_HEX is required in production (it silently falls back to a random, " +
        ...
        if (!process.env.METRICS_AUTH_TOKEN) {
            errors.push("METRICS_AUTH_TOKEN is required in production — /metrics is open without it. Generate with: openssl rand -hex 32");
        }
        // CORS must be explicitly configured in production
        if (!process.env.CORS_ALLOWED_ORIGINS) {
            errors.push("CORS_ALLOWED_ORIGINS is required in production — set it to your app/admin origins");
        }
        // Redis is required for rate limiting, sessions, and BullMQ in production
        if (!process.env.REDIS_URI) {
            errors.push("REDIS_URI is required in production — rate limiting, BullMQ queues, and session caching depend on it");
        }
        // Backup encryption must be enabled when backups are in use
        if (process.env.BACKUP_ENABLED !== "false") {
            if (!process.env.BACKUP_ENCRYPTION_KEY_HEX ||
                process.env.BACKUP_ENCRYPTION_KEY_HEX.length < 64) {
                errors.push("BACKUP_ENCRYPTION_KEY_HEX must be at least 32 bytes (64 hex chars) in production, or set BACKUP_ENABLED=false to explicitly opt out. Generate with: openssl rand -hex 32");
            }
            if (process.env.BACKUP_REQUIRE_ENCRYPTION !== "true") {
                errors.push("BACKUP_REQUIRE_ENCRYPTION must be set to 'true' in production (or set BACKUP_ENABLED=false to opt out of backups)");
            }
        }
    }
    if (errors.length > 0) {

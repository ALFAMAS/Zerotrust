...
export async function createServer() {
  ...
  if (isQueueDashboardEnabled() && process.env.REDIS_URI) {
    const dashboard = createQueueDashboard(process.env.REDIS_URI);
    if (dashboard) {
      const queueDashboard = new Hono<HonoEnv>();
      queueDashboard.use("*", authMiddleware, requireAdmin);
      queueDashboard.use("*", queueDashboardSecurity);
      queueDashboard.route("/", dashboard.app);
      app.route("/admin/queues", queueDashboard);
    }
  }

  // API reference — raw spec plus a Scalar-rendered UI at /docs.
  // openapiSpec is generated by `bun run docs:api` from route zod schemas.
  // Global securityHeaders() uses script-src 'self'; override CSP for /docs
  // so Scalar's CDN + nonce-stamped inline bootstrap can run. Production requires
  // the operator-controlled API_DOCS_ENABLED=true opt-in.
  ...
  if (apiDocsEnabled) {
    ...
    app.get(
      ...
      Scalar((c) => {
        ...
        c.header("Content-Security-Policy", buildScalarDocsCsp(nonce));
        return {
          url: "/openapi.json",
          theme: "purple",
          cdn: SCALAR_DOCS_CDN,
          nonce,
          // Avoid fonts.scalar.com so font-src can stay 'self'.

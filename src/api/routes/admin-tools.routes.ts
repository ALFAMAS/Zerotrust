...
router.post("/users/:id/impersonate", async (c) => {
  try {
    ...
    return c.json({
      ...
      expiresIn: 30 * 60,
      tokenType: "Bearer",
      impersonating: { id: target.id, email: target.email, displayName: target.displayName },
    });
  } catch (err) {
...
router.put("/users/:id/plan", async (c) => {
  try {
    ...
    const values = {
      ...
      trialEnd,
      metadata: { manualOverride: true, overriddenBy: admin.id, overriddenAt: new Date() },
    };
    ...
    await auditLog("admin.plan_override", admin.id, targetId, true, { plan, trialDays });

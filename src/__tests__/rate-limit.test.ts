...
describe("In-Memory Rate Limiter — IP banning", () => {
  ...
  it("reports IP as not banned initially", async () => {
    const { isIpBanned } = await import("../services/shared/rateLimiter/inmemory");
...
describe("Redis Rate Limiter — consumePoint", () => {
  ...
  afterEach(async () => {
    const { shutdownRedisRateLimiter } =
      await import("../services/shared/rateLimiter/redis");
  ...
  it("throws when Redis client is not initialized", async () => {
    const { consumePoint } = await import("../services/shared/rateLimiter/redis");
  ...
  it("pingRedis returns false when client is not initialized", async () => {
    const { pingRedis } = await import("../services/shared/rateLimiter/redis");
...
describe("rateLimit Hono middleware", () => {
  ...
  it("passes through when rate limiting is disabled in config", async () => {
    ...
    const app = new Hono();
    app.get("/test", rateLimit({ points: 1, windowSecs: 60 }), (c) =>
      c.json({ ok: true }),
    );
  ...
  it("allows requests within the window limit", async () => {
    const { Hono } = await import("hono");
    const { rateLimit, clearRateLimiter } =
      await import("../middleware/rateLimiting");
    clearRateLimiter();

    const app = new Hono();
    app.get("/test", rateLimit({ points: 5, windowSecs: 60 }), (c) =>
      c.json({ ok: true }),
    );
  ...
  it("returns 429 when the request limit is exceeded", async () => {
    ...
    const app = new Hono();
    app.get("/test", rateLimit({ points: 2, windowSecs: 60 }), (c) =>
      c.json({ ok: true }),
    );
  ...
  it("sets Retry-After header when blocking", async () => {
    ...
    const app = new Hono();
    app.get("/test", rateLimit({ points: 1, windowSecs: 30 }), (c) =>
      c.json({ ok: true }),
    );
  ...
  it("maintains separate buckets for different IPs", async () => {
    ...
    const blockedIp1 = await app.request("/test", {
      headers: { "x-forwarded-for": "10.0.0.4" },
    });
    const allowedIp2 = await app.request("/test", {
  ...
  it("resets counter after the time window expires", async () => {
    ...
    app.get("/test", rateLimit({ points: 1, windowSecs: 1 }), (c) =>
      c.json({ ok: true }),
    );
...
describe("configureTenantQuota and getTenantQuota", () => {
  ...
  it("stores a tenant quota", async () => {
    const { configureTenantQuota, getTenantQuota } =
      await import("../middleware/rateLimiting");
  ...
  it("allows overriding the burstAllowance", async () => {
    const { configureTenantQuota, getTenantQuota } =
      await import("../middleware/rateLimiting");
    configureTenantQuota("tenant-custom-burst", {

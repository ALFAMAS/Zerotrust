...
function makeSelectChain(finalMethod: "limit" | "offset" | "orderBy", result: unknown) {
  const chain: any = {
    from: vi.fn(() => chain),
    where: vi.fn(() => chain),
    limit: vi.fn(() => chain),
...
describe("gamification storage fallbacks", () => {
  ...
  it("returns zero points when the points ledger table is missing", async () => {
    const err = Object.assign(new Error('relation "points_ledger" does not exist'), {
      code: "42P01",
    });
    vi.mocked(getDb).mockReturnValue(makeSelectChain("limit", err) as any);

    await expect(getPointsBalance("user-1")).resolves.toBe(0);
  });

  it("returns empty point history when the points ledger table is missing", async () => {
    const err = Object.assign(new Error('relation "points_ledger" does not exist'), {
      code: "42P01",
    });
    vi.mocked(getDb).mockReturnValue(makeSelectChain("offset", err) as any);

    await expect(getPointsHistory("user-1")).resolves.toEqual({ entries: [], total: 0 });
  });
});

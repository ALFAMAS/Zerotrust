...
export const deviceAttestationMiddleware = createMiddleware<HonoEnv>(
  async (c, next) => {
    try {
      ...
      const fingerprintMatch =
        currentFingerprint.hash === sessionFingerprint?.hash;
      ...
      return c.json(
        { error: "ATTESTATION_ERROR", message: "Device verification failed" },
        500,
      );
    }
  },
...
export const requireTrustedDevice = createMiddleware<HonoEnv>(
  async (c, next) => {
    const session = c.get("session");
...
export const markDeviceAsTrusted = createMiddleware<HonoEnv>(
  async (c, next) => {
    try {
      const session = c.get("session");
      if (!session) {
        return c.json(
          { error: ErrorCodes.SESSION_NOT_FOUND, message: "No active session" },
          401,
        );
      }

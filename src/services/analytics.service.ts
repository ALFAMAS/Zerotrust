...
export async function trackFunnelEvent(event: FunnelEvent): Promise<void> {
  ...
  await db
    .update(usersTable)
    .set({ metadata: { ...meta, funnelEvents }, updatedAt: new Date() })
    .where(eq(usersTable.id, event.userId));
}
...
export async function trackFeatureEvent(
  ...
  await db
    .execute(
      sql`INSERT INTO analytics_events (id, user_id, feature, action, metadata, created_at) VALUES (gen_random_uuid(), ${userId}, ${feature}, ${action}, ${JSON.stringify(metadata ?? {})}, now())`
    )
    .catch(() => {
...
export async function getFeatureUsage(
  ...
  try {
    const rows = await db.execute(
      sql`SELECT action, count(*)::int as count FROM analytics_events WHERE feature = ${feature} AND created_at >= ${startDate} AND created_at <= ${endDate} GROUP BY action ORDER BY count DESC`
    );
    return rows as any;
  } catch {
    return [];
...
export async function logSearchQuery(
  ...
  await db
    .execute(
      sql`INSERT INTO search_analytics (id, user_id, query, result_count, source, created_at) VALUES (gen_random_uuid(), ${userId}, ${query}, ${resultCount}, ${source}, now())`
    )
    .catch(() => {
      // Table may not exist yet — silently ignore
    });
...
export async function getZeroResultQueries(
  ...
  try {
    const rows = await db.execute(
      sql`SELECT query, count(*)::int as count FROM search_analytics WHERE result_count = 0 AND created_at >= ${startDate} AND created_at <= ${endDate} GROUP BY query ORDER BY count DESC LIMIT ${limit}`
    );
    return rows as any;
  } catch {
    return [];

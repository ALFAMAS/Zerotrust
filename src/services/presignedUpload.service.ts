...
export async function generatePresignedUploadUrl(input: {
  ...
  const baseUrl = cfg.endpoint || "https://" + cfg.bucket + ".s3." + (cfg.region || "us-east-1") + ".amazonaws.com";
  ...
  const url = baseUrl + "/" + key + "?X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Credential=" + (cfg.accessKeyId || "") + "%2F" + scope + "&X-Amz-Date=" + new Date().toISOString().replace(/[-:]/g, "").slice(0, 15) + "Z&X-Amz-Expires=" + UPLOAD_TTL_SECS + "&X-Amz-SignedHeaders=host&X-Amz-Signature=" + signature;

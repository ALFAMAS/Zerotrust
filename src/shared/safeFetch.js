...
exports.assertSafeFetchHost = assertSafeFetchHost;
exports.assertSafeFetchUrl = assertSafeFetchUrl;
exports.fetchFixedUrl = fetchFixedUrl;
 ...
 */
function assertSafeFetchHost(host) {
    if (!host)
        throw new Error("SSRF guard: empty host");
    if (host.includes(":"))
        throw new Error(`SSRF guard: host must not specify a port: ${host}`);
...
function withSafeFetchDefaults(init = {}) {
 ...
 */
function fetchFixedUrl(input, init) {
 ...
 */
async function fetchPublicUrl(input, init) {

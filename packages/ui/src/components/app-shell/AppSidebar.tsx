  ...
  return (
    <aside className="flex h-full w-64 flex-col border-r border-border bg-card">
      <div className="flex h-16 shrink-0 items-center border-b border-border px-5">
        <Link href={items[0]?.href ?? "/"} onClick={onNavigate} className="flex items-center gap-2.5">
          <div
      ...
      {footer && <div className="shrink-0 space-y-1 border-t border-border px-3 py-4">{footer}</div>}
    </aside>
  );
}
  ...
  return (
    <>
      ...
      <div className="fixed inset-y-0 left-0 z-40 hidden w-64 md:block">
        <SidebarContent items={items} brandSuffix={brandSuffix} footer={footer} onNavigate={() => {}} />
      </div>
      ...
      >
        ...
        >
          <SidebarContent items={items} brandSuffix={brandSuffix} footer={footer} onNavigate={onClose} />
        </div>
      </div>
    </>
  );

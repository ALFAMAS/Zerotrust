...
| **CWE-918** SSRF | Any server-side `fetch`/HTTP whose host comes from user input (DID, webhook URL, federation provider config, image proxy, etc.) must reject IP literals, private/loopback/link-local/metadata hosts, non-default ports, and follow `redirect: "error"` with a timeout. | `assertSafeFetchHost` / `assertSafeFetchUrl` in `src/shared/safeFetch.ts` (used by `src/did/resolver.ts`, `src/federation/verify.ts`) |
...
| **CWE-532** Secrets in logs | Log only identifiers (userId, providerId, scope, client_id). Never log raw token/secret/password values. Do not put access/refresh tokens in redirect URLs — use the short-lived exchange-code pattern (see `POST /oauth/exchange`). | OAuth exchange flow in `auth.routes.ts`; magic-link GET verify in `magic-link.routes.ts` |
